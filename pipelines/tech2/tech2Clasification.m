...
for n=1:numOfSubjects
    ...
    for i = 1:length(eventSummary(n).eventQASummary)
        if (strcmp(eventSummary(n).eventQASummary(i).answer, "t"))
    ...
    EEG_STRUCT_CORRECT = pop_epoch(EEG, [], [0.1 2.2], 'eventindices' ,correctEventIndices);
    ...
    EEG_STRUCT_CORRECT.data = reshape(EEG_STRUCT_CORRECT.data,[S(1),S(2)*S(3)]);
     ...
     if (strcmp(eventSummary(n).group, "Control"))
         EEG_STRUCT_CORRECT_CONTROL = [EEG_STRUCT_CORRECT_CONTROL EEG_STRUCT_CORRECT];
     else
         EEG_STRUCT_CORRECT_DYS = [EEG_STRUCT_CORRECT_DYS EEG_STRUCT_CORRECT];
     end
...
% - Section: Calc Covariance Matrices
mat_cov_control = calcCovMat(EEG_STRUCT_CORRECT_CONTROL);
%mat_cov_dys = cov(dysU);

[controlU, controlS, controlV] = pca(EEG_STRUCT_CORRECT_CONTROL, 2);
